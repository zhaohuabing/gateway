...
- Bumped Go to 1.27.0, which includes security fixes to the `crypto/tls`, `os`, `encoding/asn1`, `encoding/xml`, `html/template`, `net/http`, `net/url`, `golang.org/x/net/idna` and `golang.org/x/net/dns/dnsmessage` packages.
...
- Added support for `MergeType` in `EnvoyExtensionPolicy` to enable route-level policies to merge with parent Gateway and ListenerSet policies.
...
- Added a `csrf` field to `SecurityPolicy` for native Cross-Site Request Forgery protection, validating the `Origin` header of mutating requests against the destination and any `additionalOrigins`, with gradual rollout via `shadowFraction`.
...
- Added support for the `filterContext` field in Lua EnvoyExtensionPolicy, allowing shared Lua scripts to be parameterized per route via `request_handle:filterContext()`.
...
- Extension server policies can now target HTTPRoutes, GRPCRoutes, HTTPRoute rules (via `sectionName`), and GRPCRoute rules (via `sectionName`) in addition to Gateways and Gateway Listeners. The extension server attachment outcome is now reported in the policy's `status.ancestors`.
...
- Added support for disabling the `crds` dependency on the gateway-helm chart via the `crds.enabled` variable.
...
- Added an `xdsNACKTotal` metric to track the number of NACKs received from Envoy, labeled by node ID and resource type URL. A NACK is a DiscoveryRequest carrying an ErrorDetail, indicating that Envoy rejected the last config update. This metric can be used to alert on config issues causing xDS rejections.
...
- Fixed duplicate CA certificates in ClientTrafficPolicy mTLS.
...
- Fixed remote JWKS fetching not allowing TLS v1.3.
- Fixed the initial fetch timing out for type.googleapis.com/envoy.extensions.transport_sockets.tls.v3.Secret.
...
- Fixed NodePort Gateway status addresses including all nodes instead of only nodes with live Envoy pods when externalTrafficPolicy is Local.
- Fixed non-xRoute backends (ext-proc, ext-auth, etc.) rejecting a mix of IP and Unix Domain Socket endpoints.
- Fixed Envoy Gateway crash-looping at startup on clusters whose Gateway API CRD bundle omits BackendTLSPolicy, such as OpenShift's Ingress-Operator-managed set, by making that watch conditional on the CRD being present. This completes the standard-channel guards added in [#9583](https://github.com/envoyproxy/gateway/pull/9583).
...
- Fixed SecurityPolicy rejecting OIDC cookieDomain values that contain a single-character label anywhere after the first label, such as example.m.com, because the validation pattern required every label but the first to be at least two characters. This applies the same fix made for Backend FQDN endpoint hostnames in [#4803](https://github.com/envoyproxy/gateway/pull/4803) to cookieDomain.
